...
export default class pvoutput extends Base {

	public adapter: Pvoutputorg | null;
	id: number;
	name: string;
	config: PvoutputorgConfig
	useWeatherAdapter: boolean;
	OID_WeatherConditions: string;


	constructor(adapter: Pvoutputorg | null, id: number, config: PvoutputorgConfig, useWeatherAdapter: boolean, OID_WeatherConditions: string) {
	...
	async getData(url: string, data: string | null): Promise<any> {
		let result = null;
		try {
			const config = {
				headers: {
	...
	public async write_EOD(): Promise<void> {
